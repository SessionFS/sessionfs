...
| `SFS_MAX_SYNC_MEMBER_BYTES_PAID` | Per-file cap inside a session archive for paid tiers (Pro/Team/Enterprise). The CLI also reads this env var to size its local pre-flight check and its unpack-time safety ceiling. **Self-hosted operators raising this cap above the 50 MB default MUST set the same value on every CLI host** — otherwise the local pull/unpack path rejects archives the server already accepted | `52428800` (50 MB) |
