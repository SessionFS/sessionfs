...
|----|---------|-----------|------------|
| HIGH-1 | Activation = required email-token flow | Rewrote §2.2 (further corrected in R3 — see below): durable attempt/verify/commit state machine with non-oracular `GET /redeem-info`, email verification via single-use short-TTL token or exact-match contact_email, admin-assisted bypass. `verification_method` recorded. **(R3 supersedes the original bind-first wording: Phase B creates the Organization row first, THEN binds `helm_licenses.org_id` — see R3 amendments.)** |
...
| MEDIUM-1 | No orphans on activation race | **SUPERSEDED BY R3 (org-before-FK ordering).** In the corrected §2.2 Phase B: the Organization row is created FIRST, then the license FK is bound with `UPDATE helm_licenses SET org_id = :org WHERE org_id IS NULL` (rowcount==0 → 409), then entitlement/OrgMember/audit — all in one transaction. Race losers roll back with zero orphans, and the FK is never violated. |
