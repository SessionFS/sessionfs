...
3. **Pin `jwks_uri` and `token_endpoint` to the issuer's origin.** The discovery doc is attacker-influenced; its advertised `jwks_uri`/`token_endpoint` MUST share the **scheme+host(+port) origin of the configured `issuer`**. A discovery doc that points `jwks_uri` at an internal or third-party host is rejected.
...
- Discovery doc advertising a `jwks_uri` on a different origin than `issuer` → rejected (origin-pin).
