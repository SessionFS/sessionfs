...
**v0.10.22 — Public Beta.** 2007 backend tests + 187 dashboard tests passing. 46 database migrations. 58 MCP tools. v0.10.22 closes long-standing org-collaboration gaps: new org members get the v0.10.0 Phase 5 design intent ("owner or org member of `project.org_id`") finally enforced in `auth.project_access` — they can now read personas, KB, wiki, tickets, and agent runs on org-scoped projects without first cloning the repo to capture a session; both invite endpoints (legacy single-org + multi-org) actually send email through `services/invite_helpers.dispatch_invite_email` (best-effort, never 500s the route); new dashboard `/invites` page with Accept + Decline; new `POST /invites/{id}/resend` (admin) and `POST /invite/{id}/decline` (recipient) with rowcount-1 guarded atomic FSM transitions; migration 046 adds `declined_at`/`decline_reason`/`last_emailed_at` to `org_invites` (strictly additive). Plus the Scout v5 multi-source signal-shape adapter contract (`docs/integrations/scout-signal-shape.md` + 4 reference n8n Code-node templates with PII scrubbing) — Scout v5 can now fan out to HN, GH Releases, Reddit, and RSS behind one workflow.
