...
**v0.10.23 — Public Beta.** 2023 backend tests + 187 dashboard tests passing. 46 database migrations. 58 MCP tools. v0.10.23 ships two operational fixes plus three release-hygiene polish items: opt-in `upsert=true` on `/entries/add` so callers with explicit `entity_ref` can supersede in place (closes Scout's silent state-cache loss); `_ConsumedResponse` sentinel in `mcp/remote_server.py` so remote MCP clients (ChatGPT, Claude.ai) actually see tools after OAuth connect (Starlette 1.x route wrapper rejects `None` return); persona name lookup case-insensitive across `start_ticket` / `create_ticket` / `update_ticket` / `list_tickets` filter plus `func.lower()`-keyed case-insensitive duplicate guard at `create_persona` (closes the `Atlas` vs `atlas` onboarding 400); release-skill Scribe-Site gate that skips invocation when `site/` is untouched since the last tag; new `.release/README.md` documenting expected develop→main merge conflicts. No migrations. No new endpoints. All fixes strictly additive — existing callers unchanged.
