...
**v0.10.21 — Public Beta.** 1985 backend tests + 187 dashboard tests passing. 45 database migrations. 58 MCP tools. v0.10.21 lands the **continuous-agent stack**: Phase 4a KB attribution (migration 045 adds `persona_name` + `author_class` + composite retrieval index to `knowledge_entries` with anti-spoof on service-key writes); the MCP `add_knowledge` equivalent (active-ticket bundle auto-threads `persona_name` and defaults `author_class=agent`); the full Scout v4 n8n integration contract (`docs/integrations/scout-n8n.md` — startup sequence, AgentRun wrap, failure branch, dedupe via stable `source_context`, smoke-test procedure); `AgentRunResponse` exposes the service-key audit triple (`actor_type`/`service_key_id`/`service_key_name`) on every read path; and `agent_runs:read` flips from "reserved" to **✅ live** so continuous agents can inspect their own run state without a human user key. Closes a starlette host-header CVE (GHSA-86qp-5c8j-p5mr) via a defensive `starlette>=1.0.1` pin. Scout v4 can now operate as a least-privilege org-bound service key — no more CEO personal-key fallback for the agent-memory loop.
