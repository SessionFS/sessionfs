...
**v0.10.20 — Public Beta.** 1959 backend tests + 187 dashboard tests passing. 44 database migrations. 58 MCP tools. v0.10.20 is Phase 3.6 of the service-key opt-in: 5 persona CRUD routes converted (`GET/POST/PUT/DELETE /personas{/name}`) into `require_scope("personas:read")` / `require_scope("personas:write")`. Migration 044 adds the 3-column audit triple to AgentPersona (mirrors v0.10.19 migration 043 shape). `assume_persona` / `forget_persona` session-state mutators remain user-key only by design. `personas:read` / `personas:write` move from "reserved" to "live". Unblocks the n8n Scout agent's runtime persona load and every future autonomous agent that needs to fetch its persona doc at runtime.
