...
**v0.10.24 — Public Beta.** 2108 backend tests + 198 dashboard tests passing. 47 database migrations. 61 MCP tools. v0.10.24 adds the Issue/Task rollup (migration 047 adds `kind` + `parent_ticket_id` to tickets so a single user-reported Issue can roll up multiple executor Tasks; full Atlas + Prism dashboard surface), closes two enterprise blockers from GH #51 (`sfs org create` 500 latent since v0.10.0 + meaningful error envelope replacing bare "Internal Server Error"), closes the GH #50 MCP persona update/delete parity gap, ships click-to-edit session title + alias inline (new `rename_session` MCP tool), adds CORS preflight support for ETag-protected writes (unblocks dashboard rules toggle), Rules-page max-tokens UX with structured envelope, and a new `user_is_project_admin` actor-side authz helper that drops user-input-trusting branches. All Codex-reviewed clean.
