...
The SessionFS MCP server exposes 68 tools, including several for project context, knowledge, rules, personas (Pro+), tickets (Team+), CI agent runs (Team+), handoffs (Pro+ for direct, Team+ for team handoffs), and autonomous work queues. When connected, AI agents can read the shared document, contribute knowledge entries, update wiki pages, fetch canonical rules, manage personas, run a ticket through its full FSM, record AgentRun outcomes, create + claim coordination handoffs with provenance carry-through, and service a work queue of tickets unattended. This works with any MCP-aware tool (Claude Code, Cursor, etc.).
