...
| POST | `/entries/add` | Add a single entry. Body: `content`, `entry_type`, `confidence`, `session_id`, `entity_ref`, `claim_class` (default: `note`). When `entity_ref` matches an active prior entry, the write is an **upsert**: similarity dedup is skipped, the new row commits, and the prior(s) are auto-dismissed with `superseded_by=<new_id>` + `supersession_reason="entity_ref upsert"`. Response includes `upserted_from: [prior_ids]` so callers can audit. Cross-project isolation: the upsert query is scoped by `project_id`, so writing the same `entity_ref` from another tenant is invisible. |
...
**Use `entity_ref` for state caches.** When an autonomous agent needs durable append-only state (e.g. an LRU map of "signals already classified"), give the entry a stable `entity_ref` like `scout-state:decisions`. Every subsequent write to `/entries/add` with the same `entity_ref` is treated as an upsert: the similarity-dedup gate is skipped (so small content deltas don't 409), the new entry commits, and the prior is auto-dismissed with `superseded_by=<new_id>`. The response surfaces `upserted_from: [prior_ids]` so the caller can confirm the roll-forward happened. Without `entity_ref` the standard 0.85 word-overlap dedup still applies — opt in by naming the slot.
