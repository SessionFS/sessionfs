...
<Aside type="note" title="Claude Code only">
Hook-based injection requires native hook support in the AI tool. As of v0.10.1, Claude Code is the only supported tool with that capability. File-based compile remains the universal path for codex, cursor, copilot, and gemini.
</Aside>
...
<Aside type="caution" title="Hook is additive, not exclusive">
If both a managed `CLAUDE.md` and the hook are present, both inject the same rules and Claude Code sees them twice. `sfs hooks install` warns when it detects this. Pick one mode unless you have a specific reason to layer them.
</Aside>
...
Cursor, Cline, Roo Code, Kilo Code, and Amp are capture-only — they are not resume targets, so resume-time sync does not apply.
