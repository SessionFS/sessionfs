...
## Self-service license activation (v0.11.0)

An organization admin can activate a license key against their org directly, with **required email verification**:

```
GET/POST /api/v1/org/activate          # Look up and begin activation of a license key
POST     /api/v1/org/activate/verify   # Confirm the emailed code to complete activation
```

Activation completes only after the admin confirms a single-use, time-limited code sent to their email address. The activation lookup is non-oracular — it never confirms whether an arbitrary key exists — and the flow is rate-limited. This gives Enterprise customers a direct path to apply a license to their org without manual back-office steps.

## Ownership transfer (v0.11.0)

Every organization has exactly one **owner**. Ownership can be transferred to another member through a two-step, owner-initiated flow:

```
POST /api/v1/org/transfer/initiate     # Current owner initiates a transfer to a target member
POST /api/v1/org/transfer/accept       # Target member accepts and becomes the new owner
POST /api/v1/org/transfer/cancel       # Initiator cancels a pending transfer
```

Both parties' standing is re-validated at accept time, and a pending transfer expires if it is not accepted. **Last-owner safety** is enforced throughout: an org can never be left without an owner, so removing or deactivating the last owner is blocked until ownership is transferred, and admins have a force-transfer path with a full audit trail.
