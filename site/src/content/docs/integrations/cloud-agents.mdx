...
> **This page covers *agent-initiated* runs** — the cloud agent itself calls the REST API. If instead a coordination layer (a script, n8n, a GitHub Action, a Claude Code session) *spawns* an external CLI agent (Codex CLI, Gemini CLI, Claude Code) to do ticket work, see [External Agent Orchestration](/integrations/external-agents) for the `create_agent_run` → spawn → `complete_agent_run` provenance wrapper.
...
If your agent *can* speak MCP, prefer it — you get 58 tools and richer descriptions. If it can't, use this layer.
...
- [External Agent Orchestration](/integrations/external-agents) — wrap a spawned Codex/Gemini/Claude Code CLI agent in an AgentRun for ticket-scoped provenance.
- [CI Integration (Agent Runs)](/integrations/ci-integration) — gate a CI build on a persona review's outcome.
